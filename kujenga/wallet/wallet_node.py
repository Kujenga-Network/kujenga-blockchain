...
from kujenga.daemon.keychain_proxy import (
    KeychainProxy,
...
from kujenga.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from kujenga.protocols import wallet_protocol
from kujenga.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from kujenga.util.byte_types import hexstr_to_bytes
from kujenga.util.check_fork_next_block import check_fork_next_block
from kujenga.util.errors import Err, ValidationError
from kujenga.util.ints import uint32, uint128
from kujenga.util.keychain import Keychain
from kujenga.util.lru_cache import LRUCache
from kujenga.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from kujenga.util.path import mkdir, path_from_root
from kujenga.wallet.block_record import HeaderBlockRecord
from kujenga.wallet.derivation_record import DerivationRecord
from kujenga.wallet.settings.settings_objects import BackupInitialized
from kujenga.wallet.transaction_record import TransactionRecord
from kujenga.wallet.util.backup_utils import open_backup_file
from kujenga.wallet.util.wallet_types import WalletType
from kujenga.wallet.wallet_action import WalletAction
from kujenga.wallet.wallet_blockchain import ReceiveBlockResult
from kujenga.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'kujenga keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSKujengaConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSKujengaConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSKujengaConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSKujengaConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSKujengaConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSKujengaConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSKujengaConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
