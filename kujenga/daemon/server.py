...
from kujenga.cmds.init_funcs import check_keys, kujenga_init
from kujenga.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from kujenga.daemon.keychain_server import KeychainServer, keychain_commands
from kujenga.daemon.windows_signal import kill
from kujenga.server.server import ssl_context_for_root, ssl_context_for_server
from kujenga.ssl.create_ssl import get_mozilla_ca_crt
from kujenga.util.kujenga_logging import initialize_logging
from kujenga.util.config import load_config
from kujenga.util.json_util import dict_to_json_str
from kujenga.util.keychain import (
    Keychain,
...
from kujenga.util.service_groups import validate_service
from kujenga.util.setproctitle import setproctitle
from kujenga.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Kujenga.")
    quit()
...
service_plotter = "kujenga plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "kujenga": "kujenga",
        "kujenga_wallet": "start_wallet",
        "kujenga_full_node": "start_full_node",
        "kujenga_harvester": "start_harvester",
        "kujenga_farmer": "start_farmer",
        "kujenga_introducer": "start_introducer",
        "kujenga_timelord": "start_timelord",
        "kujenga_timelord_launcher": "timelord_launcher",
        "kujenga_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("kujenga daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered KUJENGA_ROOT
    os.environ["KUJENGA_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with KUJENGA_ROOT: {os.environ['KUJENGA_ROOT']}")
    ...
    if service_command == "kujenga_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in kujenga_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    kujenga_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("kujenga_daemon")
