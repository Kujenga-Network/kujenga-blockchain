...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if "xkj_target_address" not in config["farmer"]:
        print(f"Setting the xkj destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "xkj_target_address" not in config["pool"]:
        print(f"Setting the xkj destination address for coinbase reward to {all_targets[0]}")
