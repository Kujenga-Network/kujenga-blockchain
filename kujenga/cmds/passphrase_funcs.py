...
from kujenga.daemon.client import acquire_connection_to_daemon
from kujenga.util.keychain import Keychain, obtain_current_passphrase, supports_os_passphrase_storage
from kujenga.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
from kujenga.util.misc import prompt_yes_no
from kujenga.util.ws_message import WsRpcMessage
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other Kujenga processes will be able to access your keys without prompting for your passphrase."
            )
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'kujenga passphrase set' or 'kujenga passphrase remove' to update or remove your passphrase")
        sys.exit(1)
