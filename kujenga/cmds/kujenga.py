...
from kujenga.cmds.configure import configure_cmd
from kujenga.cmds.farm import farm_cmd
from kujenga.cmds.init import init_cmd
from kujenga.cmds.keys import keys_cmd
from kujenga.cmds.netspace import netspace_cmd
from kujenga.cmds.passphrase import passphrase_cmd
from kujenga.cmds.plots import plots_cmd
from kujenga.cmds.show import show_cmd
from kujenga.cmds.start import start_cmd
from kujenga.cmds.stop import stop_cmd
from kujenga.cmds.wallet import wallet_cmd
from kujenga.cmds.plotnft import plotnft_cmd
from kujenga.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from kujenga.util.keychain import set_keys_root_path, supports_keyring_passphrase
from kujenga.util.ssl import check_ssl
...
@click.group(
    help=f"\n  Manage kujenga blockchain infrastructure ({__version__})\n",
    epilog="Try 'kujenga start node', 'kujenga netspace -d 192', or 'kujenga show -s'",
...
if not supports_keyring_passphrase():
    from kujenga.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show kujenga version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs kujenga daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by kujenga start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from kujenga.daemon.server import async_run_daemon
