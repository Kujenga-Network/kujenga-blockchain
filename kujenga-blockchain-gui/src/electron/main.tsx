...
import kujengaEnvironment from '../util/kujengaEnvironment';
import kujengaConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!kujengaEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    kujengaConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!kujengaConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !kujengaConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (kujengaConfig.manageDaemonLifetime()) {
        kujengaEnvironment.startKujengaDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
      ...
      },
      {
        ...
        submenu: [
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Kujenga-Network/kujenga-blockchain/issues',
              );
            },
          },
          {
    ...
    if (process.platform === 'darwin') {
      // Kujenga Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Kujenga' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Kujenga Blockchain' }),
            click: () => {
              openAbout();
            },
          },
