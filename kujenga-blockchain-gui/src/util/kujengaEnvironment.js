...
const PY_FOLDER = '../kujenga/daemon';
...
const getKujengaVersion = () => {
  let version = null;
  const exePath = getExecutablePath('kujenga');
  // first see if we can get a kujenga exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or kujenga is on the path
    try {
      version = child_process
