...
from kujenga.cmds.init_funcs import create_all_ssl, create_default_kujenga_config
from kujenga.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
from kujenga.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from kujenga.util.errors import Err
from kujenga.full_node.generator import setup_generator_args
from kujenga.full_node.mempool_check_conditions import GENERATOR_MOD
from kujenga.plotting.create_plots import create_plots, PlotKeys
...
from kujenga.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
from kujenga.consensus.condition_costs import ConditionCost
from kujenga.consensus.constants import ConsensusConstants
from kujenga.consensus.default_constants import DEFAULT_CONSTANTS
from kujenga.consensus.deficit import calculate_deficit
...
from kujenga.consensus.pot_iterations import (
    calculate_ip_iters,
...
from kujenga.consensus.vdf_info_computation import get_signage_point_vdf_info
from kujenga.full_node.signage_point import SignagePoint
from kujenga.plotting.util import PlotInfo, PlotsRefreshParameter, PlotRefreshResult, parse_plot_info
from kujenga.plotting.manager import PlotManager
from kujenga.server.server import ssl_context_for_server
...
from kujenga.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
from kujenga.types.blockchain_format.pool_target import PoolTarget
from kujenga.types.blockchain_format.program import INFINITE_COST
from kujenga.types.blockchain_format.proof_of_space import ProofOfSpace
from kujenga.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from kujenga.types.blockchain_format.sized_bytes import bytes32
from kujenga.types.blockchain_format.slots import (
...
from kujenga.types.blockchain_format.vdf import VDFInfo, VDFProof
from kujenga.types.end_of_slot_bundle import EndOfSubSlotBundle
from kujenga.types.full_block import FullBlock
from kujenga.types.generator_types import BlockGenerator, CompressorArg
...
from kujenga.util.block_cache import BlockCache
from kujenga.util.condition_tools import ConditionOpcode
from kujenga.util.config import load_config, save_config
from kujenga.util.hash import std_hash
from kujenga.util.ints import uint8, uint16, uint32, uint64, uint128
from kujenga.util.keychain import Keychain, bytes_to_mnemonic
from kujenga.util.merkle_set import MerkleSet
from kujenga.util.prev_transaction_block import get_prev_transaction_block
from kujenga.util.path import mkdir
from kujenga.util.vdf_prover import get_vdf_info_and_proof
from tests.wallet_tools import WalletTool
from kujenga.wallet.derive_keys import (
...
class BlockTools:
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="kujenga-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `kujenga generate keys`")
    ...
    async def setup_plots(self):
        ...
        try:
            ...
            plot_keys_2 = PlotKeys(self.farmer_pk, None, encode_puzzle_hash(self.pool_ph, "xkj"))
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("KUJENGA_ROOT", "~/.kujenga/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
